...
##=========================================
header <- "157_cattle_snp_geno01_maf005-ld502502_nchr"
max.k <- 8
##=========================================
...
png(file=paste(header,"admix.plot.png",sep="."),res=500, width=2000, height=1200)
par(mfrow = c(max.k, 1),mar=c(0.1,0.7,0,0),oma=c(1.5,0,0.1,0.1), mgp=c(0,0.2,0),xaxs="i",cex.lab=0.6,font.lab=2,cex.axis=0.8)
par(las=2)
...
for (i in 1:max.k){
     barplot(t(as.matrix(plot.data[[i]])),names.arg=rep(c(""),n),
             col=my.colours,border=NA,space=0,axes=F,ylab=paste("K=",i+1),xaxt="n",yaxt="n",font=1)
# plot black line for each pop in top bottom left and right
  for (i in 0:(length(plot.at1))) {
    x <- plot.at1[i]
    abline(v=0, lwd=0.6,  col="black")
    abline(v=x, lwd=0.5, col="black") 

  }
  abline(h=0, lwd=0.5, col="black")
  abline(h=1, lwd=0.6, col="black")
}
